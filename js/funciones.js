...
function operaciones(){
	...
	try{
		...
		if (Number.isNaN(n1) || Number.isNaN(n2)) {
			throw alert("Ingrese un numero o chillo ):s");
		}else if((n1<(-(2**53-1)) || n1>2**53-1) || (n2<(-(2**53-1)) || n2>2**53-1)){
			throw alert("Muy grande o muy chikito UnU");	
		}else{
...
function compararn(){
	...
	try{
		...
		if (Number.isNaN(n1) || Number.isNaN(n2) || Number.isNaN(n3)) {
			throw alert("Meta un número o chillo ):");
			...
			if (n1>n2 && n1>n3) {
				...
				document.Formulario2.mayor.value = "Son iguales ):";
			}
		}
	}catch(e){
...
function pago(){
	...
	try{
		...
		if (Number.isNaN(sueldob) || Number.isNaN(horas)) {
			throw alert("Meta un número o chillo ):");
		//Evaluar los limites
		}else if( (sueldob<120 || sueldob>10000) || (horas<30 || horas>60) ){
			throw alert("El limite en el sueldo es min 120 y max 10000 \n"+
...
function utilidad(){
	...
	try{
		...
		//Evaluar los limites
		}else if( (sueldo<4000 || sueldo>1000000) || (tiempo<=0.5 || tiempo>=70) ){
			throw alert("El limite en el sueldo es min 4000 y max 100000 \n"+
				"El limite en el tiempo por años es min 0.5 y max 70 ");
		}else{
			//Operaciones
			if(tiempo<1){
